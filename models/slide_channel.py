...
class ChannelUsersRelation(models.Model):
    ...
    _sql_constraints = [
        (
            ...
            "Mỗi sinh viên chỉ có 1 lượt đăng ký cho mỗi học phần",
        )
    ]

    channel_id = fields.Many2one(
        "slide.channel", index=True, required=True, ondelete="cascade", readonly=True
    )
    completed = fields.Boolean(
        "Is Completed",
        help="Channel validated, even if slides / lessons are added once done.",
    )
    ...
    # dac_biet = fields.Boolean("Đặc biệt")
    # chi_dat = fields.Boolean("Chỉ đạt")
    ky_dk = fields.Char("Kỳ đăng ký", size=5, readonly=True)

    duoc_phep_dang_ky = fields.Boolean(
        "Duoc phep dang ky ?", required=True, default=True
    )
    thoa_man_song_hanh = fields.Boolean(
        "Thoa man song hanh", required=True, default=True
    )
    thoa_man_truoc = fields.Boolean(
        "Thoa man truoc", required=True, default=True)
    thoa_man_tien_quyet = fields.Boolean(
        "Thoa man tien quyet", required=True, default=True
    )

    # def compute_kiem_tra_dieu_kien_dang_ky_mon(self):
    #     # TODO: triển khai logic kiểm tra xem sinh viên có được đăng ký môn này không
    #     self.duoc_phep_dang_ky = bool(random.getrandbits(1))
    #     _logger.warning("duoc phep dang ky ? %r"%(self.duoc_phep_dang_ky))
    #     # pass

    def cap_nhat_bang_channel_partner_sau_khi_user_hoan_thanh_khoa_hoc(self):
        existing_users = self.env["res.partner"].search([])
        channel_ids = self.env["slide.channel"].search([]).ids
        for user_id in existing_users.ids:
            list_mon_hoc_thoa_man_dieu_kien_mon_hoc_truoc = (
                self.env["slide.channel.partner"]
                    ...
                    [
                        ...
                        (
                            ...
                            ">=",
                            50,
                        ),  # tạm thời set là 50 để phân biệt với điều kiện của môn tiên quyết
                    ]
                )
                .channel_id
            )
            list_cac_mon_hoc_can_mon_hoc_truoc = (
                self.env["slide.channel"]
                    .sudo()
                    .search([("hp_truoc_ids", "in", channel_ids)])
            )
            # hiện tại đang gộp chung logic check môn học tiên quyết và môn học trước
            # completion = 100
            list_mon_hoc_thoa_man_dieu_kien_mon_hoc_tien_quyet = (
                self.env["slide.channel.partner"]
                    .sudo()
                    .search([("partner_id", "=", user_id), ("completion", "=", 100)])
                    .channel_id
            )
            list_cac_mon_hoc_can_mon_hoc_tien_quyet = (
                self.env["slide.channel"]
                    .sudo()
                    .search([("hp_tien_quyet_ids", "in", channel_ids)])
            )

            _logger.info(
                "list các môn cần môn học trước  %r"
                % (list_cac_mon_hoc_can_mon_hoc_truoc)
            )
            _logger.info(
                "list các môn thỏa mãn môn học trước %r"
                % (list_mon_hoc_thoa_man_dieu_kien_mon_hoc_truoc)
            )
            for mon_hoc_can_mon_hoc_truoc in list_cac_mon_hoc_can_mon_hoc_truoc:
                _logger.info(
                    "mon %r can cac mon  truoc %r"
                    % (
                        mon_hoc_can_mon_hoc_truoc.id,
                        mon_hoc_can_mon_hoc_truoc.hp_truoc_ids,
                    )
                )
                du_dieu_kien = True
                for mon_hoc_truoc in mon_hoc_can_mon_hoc_truoc.hp_truoc_ids:
                    _logger.info(
                        "mon hoc truoc cua %r la %r"
                        % (mon_hoc_can_mon_hoc_truoc, mon_hoc_truoc)
                    )
                    if (
                            mon_hoc_truoc
                            not in list_mon_hoc_thoa_man_dieu_kien_mon_hoc_truoc
                    ):
                        _logger.info(
                            "user %r chua du dieu kien dang ky mon %r "
                            % (user_id, mon_hoc_can_mon_hoc_truoc)
                        )
                        du_dieu_kien = False
                        break
                if du_dieu_kien:
                    update_val = {
                        "channel_id": mon_hoc_can_mon_hoc_truoc.id,
                        "partner_id": user_id,
                        "thoa_man_truoc": True,
                        # 'duoc_phep_dang_ky': True,
                        # 'completion': -1, # không thay đổi trạng thái môn học
                        "completed": False,
                    }
                    # self.env['slide.channel.partner'].write(update_val)
                    # phải search trước rồi mới chạy write()
                    ...
                    _logger.info(
                        "đã cập nhat dieu kien dang ki mon hoc truoc id = %r"
                        % (mon_hoc_can_mon_hoc_truoc.id)
                    )

            _logger.info(
                "list các môn cần môn học tien quyet  %r"
                % (list_cac_mon_hoc_can_mon_hoc_tien_quyet)
            )
            _logger.info(
                "list các môn thỏa mãn môn học tien quyet %r"
                % (list_mon_hoc_thoa_man_dieu_kien_mon_hoc_tien_quyet)
            )
            for (
                    mon_hoc_can_mon_hoc_tien_quyet
            ) in list_cac_mon_hoc_can_mon_hoc_tien_quyet:
                _logger.info(
                    "mon hoc %r can mon tien quyet %r"
                    % (
                        mon_hoc_can_mon_hoc_tien_quyet,
                        mon_hoc_can_mon_hoc_tien_quyet.hp_tien_quyet_ids,
                    )
                )
                du_dieu_kien = True
                for mon_tien_quyet in mon_hoc_can_mon_hoc_tien_quyet.hp_tien_quyet_ids:
                    _logger.info(
                        "mon tien quyet cua %r la %r "
                        % (mon_hoc_can_mon_hoc_tien_quyet, mon_tien_quyet)
                    )
                    if (
                            mon_tien_quyet
                            not in list_mon_hoc_thoa_man_dieu_kien_mon_hoc_tien_quyet
                    ):
                        _logger.info(
                            "user %r chua du dieu kien dang ky mon %r"
                            % (user_id, mon_hoc_can_mon_hoc_tien_quyet)
                        )
                        du_dieu_kien = False
                        break
                if du_dieu_kien:
                    update_val = {
                        "channel_id": mon_hoc_can_mon_hoc_tien_quyet.id,
                        "partner_id": user_id,
                        "thoa_man_tien_quyet": True,
                        # 'duoc_phep_dang_ky': True,
                        # 'completion': -1, # không thay đổi trạng thái học tập/tiến độ
                        "completed": False,
                    }
                    # self.env['slide.channel.partner'].write(update_val)
                    self.env["slide.channel.partner"].sudo().search(
                    ...
                    _logger.info(
                        "đã cập nhat dieu kien dang ki mon hoc tien quyet id = %r"
                        % (mon_hoc_can_mon_hoc_tien_quyet.id)
                    )
    ...
    def _recompute_completion(self):
        ...
        for item in read_group_res:
            ...
            mapped_data[item["channel_id"][0]
                        ][item["partner_id"][0]] = item["__count"]

        partner_karma = dict.fromkeys(self.mapped("partner_id").ids, 0)
        for record in self:
            record.completed_slides_count = mapped_data.get(
                record.channel_id.id, dict()
            ).get(record.partner_id.id, 0)
            record.completion = (
                100.0
                if record.completed
                else round(
            ...
            if (
                ...
                # record.completed = True # tắt tính năng tự set completed = True khi học xong hết bài học. KHi đó phải đợi admin chấm điểm + phê duyệt
                partner_karma[
                    record.partner_id.id
                ] += record.channel_id.karma_gen_channel_finish
            # TODO: chèn code kiểm tra trong bảng sinh viên _ môn học các môn học điều kiện
            # cần tối ưu tránh việc gọi hàm này liên tục mỗi khi học xon 1 bài
            # self.cap_nhat_bang_channel_partner_sau_khi_user_hoan_thanh_khoa_hoc()

        partner_karma = {
            partner_id: karma_to_add
            for partner_id, karma_to_add in partner_karma.items()
            if karma_to_add > 0
        }
        ...
        if partner_karma:
            users = (
                self.env["res.users"]
                .sudo()
                .search([("partner_id", "in", list(partner_karma.keys()))])
            )
            for user in users:
                users.add_karma(partner_karma[user.partner_id.id])
        # TODO: thêm logic update trạng thái duoc_phep_dang_ky của 1 môn học trong bảng channel_partner sau khi hoàn thành 1 khóa học bất kỳ
    ...
    def unlink(self):
        ...
        if removed_slide_partner_domain:
            ...
            _logger.info("\n removing .... %r" %
                         (removed_slide_partner_domain))
            # update_val = {
            #     # 'channel_id': mon_hoc_can_mon_sh.id,
            #     "partner_id": removed_slide_partner_domain[1][2],
            #     # tuancy Các trường comment ở dưới không có trong slide.slide.partner.
            #     # tuancy Code ở dưới update trạng thái completed chứ không xóa bản ghi slide.slide.partner
            #     # 'duoc_phep_dang_ky': True,
            #     # "completion": -1,  # không thay đổi trạng thái đăng ký/tiến độ của khóa học
            #     # "thoa_man_song_hanh": True,
            #     # "thoa_man_tien_quyet": True,
            #     # "thoa_man_truoc": True,
            #     "completed": False,
            #     "quiz_attempts_count": 0,
            # }
            # dòng dưới ulink các cái slide_slide_partner
            self.env["slide.slide.partner"].search(removed_slide_partner_domain).unlink()
        _logger.warning("\n\n removing channel_partner record\n\n")
...
class Channel(models.Model):
    ...
    _order = "sequence, id"
    # _rec_name = "ma_hp_ten_hp"
    _rec_name = "ma_hoc_phan_moi"
    _sql_constraints = [
        (
            "check_sum_trong_so_hoc_phan",
    ...
    ky_hoc_id = fields.Many2many(
        comodel_name="ky_hoc", string="Danh sách kỳ học liên quan"
    )
    id_inc = fields.Integer(
        string="Auto Inc",
        default=lambda self: self.env["ir.sequence"].next_by_code(
            "increment_id_inc"),
    )
    # channel_ids = fields.Many2many('slide.channel', 'channel_lienquan', column2='id_inc', string='Các khóa học tiên quyết')
    hp_tien_quyet_ids = fields.Many2many(
        "slide.channel",
        "slide_channel_tien_quyet",
        column2="id_inc",
        string="Các học phần tiên quyết",
    )
    hp_truoc_ids = fields.Many2many(
        "slide.channel",
        "slide_channel_truoc",
        column2="id_inc",
        string="Các học phần trước",
    )
    hp_song_hanh_ids = fields.Many2many(
        "slide.channel",
        "slide_channel_song_hanh",
        column2="id_inc",
        string="Các học phần song hành",
    )
    ...
    # Thông tin môn học
    name = fields.Char(
        compute="_compute_auto_compute_name",
        store=True,
        string="Tên học phần (cũ)",
        # size=50,
        # translate=True,
        # required=True
    )
    # đoạn này tạm sử dụng 1 trường mới vì ma_hoc_phan_cu đang liên kết với danh_muc khi
    # xóa đi thì bị lỗi keyerror
    ma_hoc_phan_moi = fields.Char(
        string="Mã học phần",
        # required=True,
    )

    ten_hoc_phan = fields.Char(string="Tên học phần", required=True,)
    # phần này tạm bỏ do không dùng danh mục học phần nữa
    ma_hoc_phan = fields.Many2one(
        comodel_name="danh_muc.hoc_phan", ondelete="set null", string="Mã học phần (cũ)"
    )
    ...
    #     related='ma_hoc_phan.ten_hoc_phan',
    #     store=True,
    #     string='Tên học phần'
    # )
    so_tin_chi = fields.Integer("Số tín chỉ", default=1, required=True)
    loai_hoc_phan = fields.Selection(
        [("1", "Bắt buộc"), ("2", "Tùy chọn")], string="Loại học phần"
    )  # cần sửa thành selection
    hoc_phan_tinh_diem = fields.Boolean(
        default=True, string="Học phần tính điểm")
    gio_tin_chi_yeu_cau = fields.Html("Giờ tín chỉ đối với các hoạt động")
    yeu_cau_ve_csvc = fields.Html("Yêu cầu về cơ sở vật chất")
    khoa_bo_mon_phu_trach = fields.Char("Khoa - bộ môn phụ trách")
    objective_kien_thuc = fields.Html("Mục tiêu về kiến thức")
    objective_ky_nang = fields.Html("Mục tiêu về kỹ năng")
    objective_thai_do = fields.Html("Thái độ - chuyên cần")
    tom_tat_noi_dung = fields.Text("Tóm tắt nội dung học phần")
    noi_dung_chi_tiet = fields.Html("Nội dung chi tiết học phần")
    hoc_lieu = fields.Html(
        "Danh sách học liệu"
    )  # cái này về sau cần link với bảng quản lý học liệu

    # trọng số điểm của học phần
    # 4 đầu điểm hiện tại làm giống với logic của trung tâm 1
    ts_attendance = fields.Float(
        default=10.0, required=True, string="Trọng số chuyên cần (%)"
    )
    ts_trung_binh_kiem_tra_tren_lop = fields.Float(
        default=10.0, required=True, string="Trọng số trung bình các bài kiểm tra (%)"
    )
    ts_bai_tap = fields.Float(
        default=10.0, required=True, string="Trọng số bài tập - thảo luận (%)"
    )
    ts_thi_nghiem = fields.Float(
        default=20.0, required=True, string="Trọng số thí nghiệm - thực hành (%)"
    )
    ts_cuoi_ky = fields.Float(
        default=50, required=True, string="Trọng số kiểm tra cuối kỳ (%)"
    )

    @api.constrains("ts_attendance", "ts_trung_binh_kiem_tra_tren_lop", "ts_bai_tap", "ts_thi_nghiem", "ts_cuoi_ky")
    def _kiem_tra_trong_so(self):
        for record in self:
            if (record.ts_attendance + record.ts_trung_binh_kiem_tra_tren_lop + record.ts_bai_tap + record.ts_thi_nghiem + record.ts_cuoi_ky) != 100:
                raise models.ValidationError("Tổng trọng số các đầu điểm khác 100%!")

    # trọng số điểm của học phần - kiểu động
    # số lượng trọng số điểm không cố định, tùy theo hình thức đào tạo
    trong_so_diem_hoc_phan_ids = fields.One2many(
        comodel_name="qldt.trong_so_diem_hoc_phan",
        inverse_name="hoc_phan_ap_dung_id",
        # compute='_compute_lay_danh_sach_trong_so_diem_theo_hinh_thuc_dao_tao',
        # store=True,
        string="Danh sách trọng số",
    )
    ...
    # thông tin về ngành học + chuyên ngành
    nganh_id = fields.Many2many(
        comodel_name="quan_ly_nganh_hoc.nganh",
        relation="quan_ly_nganh_hoc_mon_hoc_nganh",
        column2="id",
        string="Ngành",
    )
    chuyen_nganh_id = fields.Many2many(
    ...
    hoc_lieu_ids = fields.Many2many(
        comodel_name="slide.slide", string="Danh sách học liệu"
    )

    lop_tin_chi_ids = fields.One2many(
        comodel_name="lop_tin_chi",
        inverse_name="mon_hoc_ids",
        # column2='lop_tin_chi_ids'
        string="Danh sách lớp tín chỉ",
    )

    # lop_boi_duong_ids = fields.One2many(
    #     comodel_name="qldt.lop_boi_duong",
    #     inverse_name="slide_channel_id",
    #     string="Danh sách lớp bồi dưỡng"
    # )

    khoa_id = fields.Many2one(
        comodel_name="danh_muc.khoa", string="Khoa phụ trách")

    # thông tin về môn học điều kiện tương ứng với từng ngành - chuyên ngành
    mon_hoc_dieu_kien_tien_quyet_ids = fields.Many2many(
        comodel_name="mon_hoc_dieu_kien", relation="mon_hoc_tien_quyet"
    )

    mon_hoc_dieu_kien_truoc_ids = fields.Many2many(
        comodel_name="mon_hoc_dieu_kien", relation="mon_hoc_truoc"
    )

    mon_hoc_dieu_kien_song_hanh_ids = fields.Many2many(
        comodel_name="mon_hoc_dieu_kien", relation="mon_hoc_song_hanh"
    )

    # slides: promote, statistics
    slide_ids = fields.One2many(
        "slide.slide", "channel_id", string="Slides and categories"
    )
    slide_content_ids = fields.One2many(
        "slide.slide", string="Slides", compute="_compute_category_and_slide_ids"
    )
    ...
    nbr_presentation = fields.Integer(
        "Presentations", compute="_compute_slides_statistics", store=True
    )
    ...
    nbr_webpage = fields.Integer(
        "Webpages", compute="_compute_slides_statistics", store=True
    )
    nbr_quiz = fields.Integer(
        "Number of Quizs", compute="_compute_slides_statistics", store=True
    )
    nbr_slink_video = fields.Integer(
        "Number of Slink video", compute="_compute_slides_statistics", store=True
    )
    total_slides = fields.Integer(
        "Content", compute="_compute_slides_statistics", store=True
    )
    ...
    members_count = fields.Integer(
        "Attendees count", compute="_compute_members_count")
    members_done_count = fields.Integer(
        "Attendees Done Count", compute="_compute_members_done_count"
    )
    ...
    is_member = fields.Boolean(
        string="Is Member", compute="_compute_is_member", compute_sudo=False
    )
    ...
    karma_gen_channel_finish = fields.Integer(
        string="Course finished", default=10)
    # Karma based actions
    ...
    url_danh_sach_hoc_lieu = fields.Char(
        compute="_compute_url_danh_sach_hoc_lieu",
        store=True,
        string="URL danh sách học liệu"
    )
    kieu_tinh_diem_quiz = fields.Selection(
        [("1", "Làm đến khi đúng hết"), ("2", "Chỉ làm 1 lần và đánh dấu là hoàn thành")],
        default="1",
        string="Kiểu làm bài Quiz",
        required=True,
        help="Chọn các kiểu làm Quiz",
    )

    @api.depends("ma_hoc_phan_moi","ten_hoc_phan")
    def _compute_ma_hp_ten_hp(self):
        for record in self:
            if record.ma_hoc_phan_moi and record.ten_hoc_phan:
                record.ma_hp_ten_hp = record.ma_hoc_phan_moi + '-' + record.ten_hoc_phan

    @api.depends('slide_ids')
    def _compute_url_danh_sach_hoc_lieu(self):
        for record in self:
            """
                TODO:
                    1. URL hàm này đang hardcode, cần sửa
            """
            base_url = record.get_base_url()
            record.url_danh_sach_hoc_lieu = "/slides/%s" % (slug(record))
            _logger.info("\n\n\n getting slide_slugs = {}".format(
                record.url_danh_sach_hoc_lieu))

    @api.onchange("ts_cuoi_ky")
    def validate_ts_cuoi_ky(self):
        if self.ts_cuoi_ky < 50:
            raise ValidationError(
                "Trọng số điểm kiểm tra cuối kỳ không được thấp hơn 50%!"
            )

    def load_slide(self):
        # print("load học liệu nào")
        # print(self.id)
        # print(request.env.user.partner_id.id)
        vls = self.env['slide.slide.partner'].sudo().search([
            ('channel_id.id', '=', self.id),
            ('partner_id.id', '=', request.env.user.partner_id.id)])
        # for vl in vls:
        # print(vl.slide_id.name)
        return vls

    def sinh_vien_mon_hoc(self):
        vls = self.env['slide.channel.partner'].sudo().search([
            ('channel_id.id', '=', self.id),
            ('partner_id.id', '=', request.env.user.partner_id.id)])
        # so_slide = vls.completed_slides_count
        # print(vls)
        return vls

    def danh_sach_sinh_vien_mon_hoc(self):
        vls = self.env['slide.channel.partner'].sudo().search([
            ('channel_id.id', '=', self.id)])
        # so_slide = vls.completed_slides_count
        # print(vls)
    ...
    def so_quiz_hoan_thanh(self):
        ...
        for vl in vls:
            if vl.slide_id.question_ids and vl.completed:
                quiz_done = quiz_done + 1
            if vl.slide_id.question_ids:
                quiz_sub = quiz_sub + vl.quiz_attempts_count
        # so_quiz = str(quiz_done + "/" + quiz_total)
        # print(so_quiz)
        d = dict();
        d['quiz_done'] = quiz_done
        d['quiz_total'] = quiz_total
        d['quiz_sub'] = quiz_sub
        return d

        # so_slide = vls.completed_slides_count
        # print(vls)
    # @api.constrains('so_tin_chi')
    # def validate_so_tin_chi(self):
    #     if self.so_tin_chi <1 or self.so_tin_chi > 4:
    #         raise ValidationError('Số tín chỉ của một học phần không được nhỏ hơn 1 hoặc lớn hơn 4!')

    @api.depends("ten_hoc_phan")
    def _compute_auto_compute_name(self):
        for record in self:
            try:
                record.name = record.ten_hoc_phan
    ...
    def _compute_members_count(self):
        read_group_res = (
            self.env["slide.channel.partner"]
                .sudo()
                .read_group([("channel_id", "in", self.ids)], ["channel_id"], "channel_id")
        )
        data = dict(
            (res["channel_id"][0], res["channel_id_count"]) for res in read_group_res
        )
        for channel in self:
    ...
    def _compute_is_member(self):
        # _logger.info("\n logging from _compute_is_member")
        channel_partners = (
            self.env["slide.channel.partner"]
                .sudo()
                .search(
        ...
        # _logger.warning('\ncurrent channel_partner result : %r' % (result))
        for channel in self:
            channel.is_member = (
                channel.is_member
            ) = self.env.user.partner_id.id in result.get(channel.id, [])
        # _logger.warning("\n\n checking if a partner is joined in this channel... %r" % (channel.is_member))

    # @api.model
    def compute_is_quan_tri_vien(self):
        if request.env.user.has_group('website_slides.group_website_quan_tri_vien_tu_xa') \
                or request.env.user.has_group('website_slides.group_website_quan_tri_vien_chinh_quy') \
                or request.env.user.has_group('website_slides.group_website_quan_tri_vien_vlvh') \
                or request.env.user.has_group('website_slides.group_website_quan_tri_vien_bd') \
                or request.env.user.has_group('website_slides.group_website_quan_tri_vien') \
                or request.env.user.has_group('website_slides.group_website_slides_officer'):
            return True
        return False

    def compute_is_tro_giang(self):
        if request.env.user.has_group('website_slides.group_website_slides_tro_giang'):
            return True
        return False
    ...
    def _compute_slides_statistics(self):
        default_vals = dict(total_views=0, total_votes=0,
                            total_time=0, total_slides=0)
        keys = [
            "nbr_%s" % slide_type
            for slide_type in self.env["slide.slide"]
            ._fields["slide_type"]
            .get_values(self.env)
        ]
    ...
    def _compute_slides_statistics_type(self, read_group_res):
        ...
        result = dict(
            (cid, dict((key, 0) for key in keys + ["total_slides"])) for cid in self.ids
        )
        for res_group in read_group_res:
            cid = res_group["channel_id"][0]
        ...
        for record in self:
            if record.user_id == self.env.user or self.env.is_superuser():
                ...
                record.can_upload = bool(
                    record.upload_group_ids & self.env.user.groups_id
                )
            else:
    ...
    def _compute_website_url(self):
        ...
        for channel in self:
            if (
                ...
                channel.website_url = "%s/slides/%s" % (
                    base_url, slug(channel))
    ...
    def create(self, vals):
        ...
        # đoạn code này, nếu để lại thì nó sẽ tự chèn vào bảng channel_partner 1 bản ghi => thừa và
        # không đúng với logic populate sinh viên - môn học khi có môn học mới được thêm vào
        # if not vals.get('channel_partner_ids') and not self.env.is_superuser():
        # vals['channel_partner_ids'] = [(0, 0, {
        #     'partner_id': self.env.user.partner_id.id
        # })]

        # kiểm tra nếu trọng số điểm đã tồn tại thì ghi đè
        if vals.get('ma_hoc_phan_moi'):
            existed_channel = self.env['slide.channel'].search([
                ('ma_hoc_phan_moi', '=', vals.get('ma_hoc_phan_moi')),
            ])
            if len(existed_channel) == 1:
                res = super(Channel, existed_channel).write({
                    'ts_attendance': vals.get('ts_attendance'),
                    'ts_bai_tap': vals.get('ts_bai_tap'),
                    'ts_thi_nghiem': vals.get('ts_thi_nghiem'),
                    'ts_trung_binh_kiem_tra_tren_lop': vals.get('ts_trung_binh_kiem_tra_tren_lop'),
                    'ts_cuoi_ky': vals.get('ts_cuoi_ky'),
                })
                return existed_channel
            # handle trường hợp nếu có nhiều học phần trùng mã thì chỉ update học phần mới nhất
            elif len(existed_channel) > 1:
                res = super(Channel, existed_channel[-1]).write({
                    'ts_attendance': vals.get('ts_attendance'),
                    'ts_bai_tap': vals.get('ts_bai_tap'),
                    'ts_thi_nghiem': vals.get('ts_thi_nghiem'),
                    'ts_trung_binh_kiem_tra_tren_lop': vals.get('ts_trung_binh_kiem_tra_tren_lop'),
        ...
        channel = super(
            Channel, self.with_context(mail_create_nosubscribe=True)
        ).create(vals)
        ...
        # đoạn này tạm comment out nhưng cần xem lại vì có liên quan đến phân quyền sau này
        if "enroll_group_ids" in vals:
            channel._add_groups_members()

        # existing_users = self.env['res.partner'].search([])

        # vòng for này duyệt từng người dùng trong danh sách
        # với mỗi người dùng, kiểm tra danh sách:
        #   - môn đã hoàn thành
        #   - môn đẵ đăng ký
        #   - môn đã học xong
        # sau đó đối chiếu từng danh sách trên với: 3 danh sách môn tiên quyết, môn hoc trước, môn song hành của môn hiện tại (môn đang định add vào DB)
        # nếu 03 danh sách môn học của người dùng không đáp ứng được đủ điều kiện của môn định add vào DB thì người dùng đó không được đăng ký môn này
        # tức là trường duoc_phep_dang_ky = False
        # sau đó ghi bản ghi tương ứng vào bảng channel_partner aka sinhvien mon hoc
        # for user_id in existing_users.ids:
        #     duoc_phep_dang_ky = True
        #     thoa_man_tien_quyet = True
        #     thoa_man_song_hanh = True
        ...
        # # logic kiểm tra môn điều kiện theo từng môn - tạm dừng
        # list_mon_hoc_da_hoan_thanh = self.env['slide.channel.partner'].sudo().search(
        #     [('partner_id', '=', user_id), ('completed', '=', True)])
        # list_mon_hoc_da_dang_ky = self.env['slide.channel.partner'].sudo().search(
        #     [('partner_id', '=', user_id), ('completion', '=', 0)])
        # list_mon_hoc_da_hoc_xong = self.env['slide.channel.partner'].sudo().search(
        #     [('partner_id', '=', user_id), ('completion', '=', 100)])
        #
        # mon_hoc_tien_quyet_cua_mon_hien_tai = vals['hp_tien_quyet_ids'][0][2]
        # mon_hoc_song_hanh_cua_mon_hien_tai = vals['hp_song_hanh_ids'][0][2]
        # mon_hoc_truoc_cua_mon_hien_tai = vals['hp_truoc_ids'][0][2]
        #
        # for mon_tien_quyet in mon_hoc_tien_quyet_cua_mon_hien_tai:
        #     if mon_tien_quyet not in list_mon_hoc_da_hoan_thanh.channel_id.ids:
        #         duoc_phep_dang_ky = False
        #         thoa_man_tien_quyet = False
        # for mon_song_hanh in mon_hoc_song_hanh_cua_mon_hien_tai:
        #     if mon_song_hanh not in list_mon_hoc_da_dang_ky.channel_id.ids:
        #         duoc_phep_dang_ky = False
        #         thoa_man_song_hanh = False
        # for mon_hoc_truoc in mon_hoc_truoc_cua_mon_hien_tai:
        #     if mon_hoc_truoc not in list_mon_hoc_da_hoc_xong.channel_id.ids:
        #         duoc_phep_dang_ky = False
        #         thoa_man_truoc = False

        # val_list = [
        #     {
        #         'channel_id': channel.id,
        #         'partner_id': user_id,
        #         'duoc_phep_dang_ky': duoc_phep_dang_ky,
        #         'thoa_man_song_hanh': thoa_man_song_hanh,
        #         'thoa_man_truoc': thoa_man_truoc,
        #         'thoa_man_tien_quyet': thoa_man_tien_quyet,
        #         'completion': -1,
        #         'completed': False
        #     }
        # ]
        # self.env['slide.channel.partner'].create(val_list)
        return channel

    def duoc_phep_dang_ky(self):
        current_user_id = self.env.user.partner_id.id
        current_channel_id = self.id
        result = (
            self.env["slide.channel.partner"]
                .sudo()
                .search(
        ...
        # TODO: tim user khong co thi tao n khoa hoc * 1 user ban ghi trong channel_partner
        _logger.warning("\n\nresult %r\n\n" % (result))
        # _logger.warning("result.thoa_man_song_hanh %r" % (result.thoa_man_song_hanh))
        # _logger.warning("result.thoa_man_truoc %r" % (result.thoa_man_truoc))
        # _logger.warning("result.thoa_man_tien_quyet %r" % (result.thoa_man_tien_quyet))
        return (
            result.thoa_man_song_hanh
            and result.thoa_man_truoc
            and result.thoa_man_tien_quyet
        )

    """
        Hàm này được kích hoạt khi có thay đổi trong bảng sinhvien_mon hoc
        để kiểm tra điều kiện đăng ký các môn học tiên quyết 
    """

    def cap_nhat_bang_channel_partner_sau_khi_user_dang_ky_khoa_hoc(self):
        existing_users = self.env["res.partner"].search([])
        channel_ids = self.env["slide.channel"].search([]).ids
        mon_du_dieu_kien = []
        for user_id in existing_users.ids:
            duoc_phep_dang_ky = True
            list_mon_hoc_thoa_man_dieu_kien_song_hanh = (
                self.env["slide.channel.partner"]
                    .sudo()
                    .search([("partner_id", "=", user_id), ("completion", ">=", 0)])
                    .channel_id
            )
            list_cac_mon_hoc_can_mon_song_hanh = (
                self.env["slide.channel"]
                    .sudo()
                    .search([("hp_song_hanh_ids", "in", channel_ids)])
            )
            # _logger.info('list các môn cần môn song hành: %r' % (list_cac_mon_hoc_can_mon_song_hanh))
            # _logger.info("list các môn thỏa mãn song hành %r" % (list_mon_hoc_thoa_man_dieu_kien_song_hanh))
            for mon_hoc_can_mon_sh in list_cac_mon_hoc_can_mon_song_hanh:
                _logger.info(
                    "mon %r can cac mon song hanh %r"
                    % (mon_hoc_can_mon_sh.id, mon_hoc_can_mon_sh.hp_song_hanh_ids)
                )
                du_dieu_kien = True
                for mon_sh in mon_hoc_can_mon_sh.hp_song_hanh_ids:
                    _logger.info(
                        "mon song hanh cua %r la %r" % (
                            mon_hoc_can_mon_sh, mon_sh)
                    )
                    if mon_sh not in list_mon_hoc_thoa_man_dieu_kien_song_hanh:
                        _logger.info(
                            "user %r chua du dieu kien dang ky mon %r "
                            % (user_id, mon_hoc_can_mon_sh)
                        )
                        du_dieu_kien = False
                        break
                if du_dieu_kien:
                    ...
                    # self.env['slide.channel.partner'].write(update_val)
                    # thử search rồi mới update vì để write() không như dòng trên thì ko chạy update vào db
                    self.env["slide.channel.partner"].sudo().search(
                        [
                            ("channel_id", "=", mon_hoc_can_mon_sh.id),
                            ("partner_id", "=", user_id),
                    ...
                    _logger.info(
                        "đã cập nh dieu kien dang ki, channel_id = %r"
                        % (mon_hoc_can_mon_sh.id)
                    )

    def write(self, vals):
        # đây là hàm update, trong Odoo, CRUD tương ứng là : create, read, write, unlink
        # If description_short wasn't manually modified, there is an implicit link between this field and description.
        if (
                vals.get("description")
                and not vals.get("description_short")
        ...
        _logger.warning(
            "\n\n updating existing record in channel_ AKA monhoc.hocphan\n"
        )
        return res
    ...
    def toggle_active(self):
        ...
        if to_activate:
            to_activate.with_context(active_test=False).mapped(
                "slide_ids"
            ).action_unarchive()
    ...
    def message_post(self, *, parent_id=False, subtype_id=False, **kwargs):
        ...
        return super(Channel, self).message_post(
    ...
    def _action_add_members(self, target_partners, **member_values):
        ...
        """
        _logger.info("\n\nlogging from _action-add_members...")
        # _logger.info("user id = %r co duoc dang ki hoa hoc %r khong ? %r"
        #              % (self.env.user.partner_id.id, self.id, self.duoc_phep_dang_ky()))
        ...
        if to_join:
            ...
            # _logger.info("so khớp danh sách partner_id đang add vào với danh sách đã có trong bảng sinhvien_monhoc,\
            # để xem trong DB đã có những cặp nào rồi existing map = %r" % (existing_map))
            # _logger.info("danh sách partner_id đang định add vào : target_partners = %r" % (target_partners))
            # _logger.info("channel định join : to_join = %r" % (to_join))
            # _logger.info("con trỏ **member_values = %r" % (member_values))
            to_create_values = [
                dict(
                    channel_id=channel.id,
                ...
                # for partner in target_partners if partner.id not in existing_map[channel.id] code cũ mặc định của Odoo
            ]

            # đoạn này đang gặp phải vấn đề sau:
            # theo logic bình thường khi người dùng án nút JOIN course trên web, giao diện sẽ thay đổi -> you're enrolled
            # đồng thời trong DB thì recorđ có (partner_id,channel_id) tương ứng thì được update completion = 0
            # tuy nhiên để update được completion bằng 0 thì cần search đúng bản ghi cần thay đổi, sau đó mới có thể thực thi update
            # qua hàm write()
            # nếu chỉ gọi self.env['slide.channel.partner'].write(values) thì không update được
            # chưa handle luồng unenroll course
            slide_partners_sudo = True
            #
            for each_record in to_create_values:
                _logger.info("các bản ghi sắp được thêm vào.....%r" %
                             (each_record))
                current_channel_partner_op = (
                    self.env["slide.channel.partner"]
                        .sudo()
                        .search(
                        [
                ...
                # _logger.info("kết quả update vào DB  : %r" % (slide_partners_sudo))
                if slide_partners_sudo:
                    _logger.info(
                        "ket qua search theo channel_id %r"
                        % (
                            self.env["slide.channel.partner"]
                            .sudo()
                            .search(
                    ...
                    _logger.info(
                        "đã đăng ký thành công khóa học, cần kiểm tra + update lại bảng svmh"
                    )
                    # self.cap_nhat_bang_channel_partner_sau_khi_user_dang_ky_khoa_hoc()

            # slide_partners_sudo = self.env['slide.channel.partner'].sudo().create(to_create_values)
            # code cũ, dùng create(), mỗi khi sinh viên enroll 1 khóa học thì tạo 1 bản ghi mới trong bảng sinhvien_monhoc
            # nếu sinh viên học xong thì completion = 100%, logic này không phù hợp với logic của mình, vì logic của mình là:
            # tạo mới 1 khóa học -> tự populate m*n bản ghi vào bảng sinhvien_monhoc.
            # Nếu sinh viên đăng ký khóa học -> update trạng thái completion = 0 vào bảng sinhvien_monhoc
            # do đó, cần dùng hàm write() để update thay vì dùng create() để tạo mới

            # _logger.info("\n\n update bảng channel_partner, set completion = 0 vaà completed = false để đánh dấu là môn này đã được đăng ký nhưng chưa học xong")
    ...
    def _get_earned_karma(self, partner_ids):
        ...
        for partner_channel in channel_completed:
            ...
            total_karma[
                partner_channel.partner_id.id
            ] += channel.karma_gen_channel_finish
    ...
    def _remove_membership(self, partner_ids):
        ...
        users = (
            self.env["res.users"]
            .sudo()
            .search([("partner_id", "in", list(earned_karma)), ])
        )
        for user in users:
        ...
        for channel in self:
            if channel.id not in requested_cids:
                activities += channel.activity_schedule(
                    "website_slides.mail_activity_data_access_request",
                    note=_("<b>%s</b> is requesting access to this course.")
                    % partner.name,
    ...
    #     result = []
    #     for record in self:
    #         rec_name = f"{record.ma_hoc_phan_moi} - {record.name}"
    ...
    def _name_search(self, name, args=None, operator="ilike", limit=100, name_get_uid=None):
        ...
        domain = []
        if name:
            domain = ["|", ('ma_hoc_phan_moi', operator, f"%{name}%"), ('name', operator, f"%{name}%")]
...
# @api.depends('phuong_phap_danh_gia_hoc_phan_id','trong_so_diem_hoc_phan_ids')
# def _compute_lay_danh_sach_trong_so_diem_theo_hinh_thuc_dao_tao(self):
#     for record in self:
#         if record.phuong_phap_danh_gia_hoc_phan_id:
#             danh_sach_dau_diem = self.env['danh_muc.dau_diem'].search([
#                 ('phuong_phap_danh_gia_hoc_phan_id','in',record.phuong_phap_danh_gia_hoc_phan_id.ids)
#             ])
#             danh_sach_trong_so_diem = self.env['qldt.trong_so_diem_hoc_phan'].search([
#                 ('hoc_phan_ap_dung_id','=',record.id),
#                 ('dau_diem_id','in',danh_sach_dau_diem.ids)
#             ])
#             if len(danh_sach_dau_diem) > len(danh_sach_trong_so_diem):
#                 for each_dau_diem in danh_sach_dau_diem:
#                     self.env['qldt.trong_so_diem_hoc_phan'].create({
#                         'hinh_thuc_dao_tao_id':record.hinh_thuc_dao_tao_id.id,
#                         'hoc_phan_ap_dung_id':record.id,
#                         'dau_diem_id':each_dau_diem.id,
...
#                 ('dau_diem_id', 'in', danh_sach_dau_diem.ids)
#             ])
#             _logger.info("danh sách đầu điểm : {} \n\n danh sách trọng số điểm : {}\n\n".format(
#                 danh_sach_dau_diem,danh_sach_trong_so_diem
#             ))
#             record.trong_so_diem_hoc_phan_ids = danh_sach_trong_so_diem
