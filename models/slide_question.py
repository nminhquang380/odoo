...
class SlideQuestion(models.Model):
    ...
    def _compute_statistics(self):
        slide_partners = (
            ...
            .sudo()
            .search([("slide_id", "in", self.slide_id.ids)])
        )
        slide_stats = dict(
            (
                s.slide_id.id,
                dict({"attempts_count": 0, "attempts_unique": 0, "done_count": 0}),
            )
            for s in slide_partners
        )

        for slide_partner in slide_partners:
            slide_stats[slide_partner.slide_id.id][
                "attempts_count"
        ...
        for question in self:
            ...
            question.attempts_avg = (
                stats.get("attempts_count", 0) / stats.get("attempts_unique", 1)
                if stats
                else 0
            )
...
class SlideAnswer(models.Model):
    ...
    question_id = fields.Many2one(
        "slide.question", string="Question", required=True, ondelete="cascade"
    )
...
class slide_partner_qa(models.Model):
    _name = "slide.partner.qa"
    _description = "Slide Question's Answer"


    question_id = fields.Many2one(
        "slide.question", string="Question", required=True, ondelete="cascade")
    answer_ids = fields.Many2one("slide.answer", string="Answer")
    partner_id = fields.Many2one(
        "res.partner", index=True, required=True, ondelete="cascade"
    )
    quiz_attempts_count = fields.Integer("quiz attempts count")
    ...
    slide_slide_partner_id = fields.Many2one("slide.slide.partner", ondelete="cascade")
    is_correct = fields.Boolean("Is correct answer")
