...
from .constants_user_groups import private_key_path, key_id, cloudfront_path, thoi_gian_het_han_link
...
class SlidePartnerRelation(models.Model):
    ...
    slide_id = fields.Many2one(
        "slide.slide", ondelete="cascade", index=True, required=True
    )
    ...
    slide_partner_qa_id = fields.One2many(
        comodel_name="slide.partner.qa",
        inverse_name="slide_slide_partner_id",
    )
...
class EmbeddedSlide(models.Model):
    ...
    slide_id = fields.Many2one(
        "slide.slide", string="Presentation", required=True, index=True
    )
    ...
    def _add_embed_url(self, slide_id, url):
        ...
        if not baseurl:
            return 0
        embeds = self.search(
            [("url", "=", baseurl), ("slide_id", "=", int(slide_id))], limit=1
        )
        if embeds:
...
class Slide(models.Model):
    ...
    thong_tin_phan_hoc = fields.Html("Thông tin phần học", translate=True)
    ...
    completion_time = fields.Float(
        "Duration", digits=(10, 4), help="The estimated completion time for this slide"
    )
    # Categories
    ...
    ke_tiep_cua = fields.Many2one('slide.slide', string="Học liệu tiên quyết", ondelete="cascade", store=True) #TODO filter chỉ các slide trong khóa học
    is_slink_video = fields.Boolean("Sử dụng Slink video", help="Liên kết của Slink video")
    slink_video_url = fields.Char("Key Slink video")
    ...
    questions_count = fields.Integer(
        string="Numbers of Questions", compute="_compute_questions_count"
    )
    ...
    link_ids = fields.One2many(
        "slide.slide.link", "slide_id", string="External URL for this slide"
    )
    slide_resource_ids = fields.One2many(
        "slide.slide.resource", "slide_id", string="Additional Resource for this slide"
    )
    ...
    likes = fields.Integer(
        "Likes", compute="_compute_user_info", store=True, compute_sudo=False
    )
    dislikes = fields.Integer(
        "Dislikes", compute="_compute_user_info", store=True, compute_sudo=False
    )
    user_vote = fields.Integer(
        "User vote", compute="_compute_user_info", compute_sudo=False
    )
    ...
    total_views = fields.Integer(
        "Views", default="0", compute="_compute_total", store=True
    )
    # comments
    ...
    nbr_presentation = fields.Integer(
        "Number of Presentations", compute="_compute_slides_statistics", store=True
    )
    ...
    nbr_infographic = fields.Integer(
        "Number of Infographics", compute="_compute_slides_statistics", store=True
    )
    nbr_webpage = fields.Integer(
        "Number of Webpages", compute="_compute_slides_statistics", store=True
    )
    ...
    @api.onchange('ke_tiep_cua')
    def compute_ke_tiep_cua(self):
        self.ke_tiep_cua = self.ke_tiep_cua
    ...
    def _compute_slide_views(self):
        ...
        mapped_data = dict(
            (res["slide_id"][0], res["slide_id_count"]) for res in read_group_res
        )
        for slide in self:
    ...
    def _compute_embed_code(self):
        ...
        for record in self:
            ...
            elif record.slide_type == 'video' and record.document_id and not record.is_slink_video:
            ...
            elif record.slide_type == 'video' and record.is_slink_video:
                # print("tính iframe :V", record.name)
                object_path = urllib.parse.quote(record.slink_video_url)
                url_get = self.get_url_video_slink(self.rsa_signer, object_path)
                record.embed_code = '<iframe id="ytplayer" src="%s" allowFullScreen="true" frameborder="0"></iframe>' % (url_get)
                # record.document_id = "AISoft" #Cái này để fake cho hệ thống chạy cái player video :v <hiện tại không cần nữa>
            else:
                record.embed_code = False

    def get_url_video_slink(self, rsa_signer, object_path):
        cloudfront_signer = CloudFrontSigner(key_id, rsa_signer)

        # Create a signed url that will be valid until the specific expiry date
        # provided using a canned policy.
        # expire_date = datetime.datetime(2022, 1, 1)
        print(datetime.datetime.now())
        expire_date = datetime.datetime.now() + timedelta(hours= thoi_gian_het_han_link)
        print(expire_date)
        signed_url = cloudfront_signer.generate_presigned_url(
            cloudfront_path + object_path, date_less_than=expire_date)
        # print(signed_url)
        return signed_url
    ...
    @api.onchange('is_slink_video')
    def xoa_url_thuong_khi_chon_slink_video(self):
        if self.is_slink_video:
            self.url = None
