...
class WebsiteSlides(WebsiteProfile):
    ...
    def _set_completed_slide(self, slide):
        ...
        if slide.slide_type == 'quiz' or slide.question_ids:
            raise werkzeug.exceptions.Forbidden(
                _("Slide with questions must be marked as done when submitting all good answers "))
    ...
    def _get_slide_detail(self, slide):
        ...
        if slide.channel_id.channel_type == 'documentation':
            ...
            uncategorized_domain = expression.AND(
                [base_domain, [('channel_id', '=', slide.channel_id.id), ('category_id', '=', False)]])
        ...
        if self.check_dieu_kien_hoc(slide):
            ...
            values = {
                ...
                'is_sinh_vien': request.env.user._is_sinh_vien(),
                # rating and comments
            ...
            slide_index = ''
            previous_slide = slide.channel_id.slide_content_ids[slide_index - 1] if slide_index > 0 else None
            next_slide = slide.channel_id.slide_content_ids[slide_index + 1] if slide_index < len(
    ...
    def _get_channel_slides_base_domain(self, channel):
        ...
        base_domain = expression.AND(
            [request.website.website_domain(), ['&', ('channel_id', '=', channel.id), ('is_category', '=', False)]])
        if not channel.can_publish:
            if request.website.is_public_user():
                base_domain = expression.AND([base_domain, [('website_published', '=', True)]])
            else:
                base_domain = expression.AND(
                    [base_domain, ['|', ('website_published', '=', True), ('user_id', '=', request.env.user.id)]])
        return base_domain
            ...
            for slide_partner in slide_partners:
                ...
                if slide_partner.slide_id.question_ids:
                    ...
                    channel_progress[slide_partner.slide_id.id]['quiz_gain'] = gains[
                        slide_partner.quiz_attempts_count] if slide_partner.quiz_attempts_count < len(gains) else gains[
                        -1]
    ...
    def channel(self, channel, category=None, tag=None, page=1, slide_type=None, uncategorized=False, sorting=None,
        ...
        domain_khoa_hoc = request.website.website_domain()
        channels_all = request.env['slide.channel'].search(domain_khoa_hoc)
        channels_popular = channels_all.sorted('total_votes', reverse=True)[:3]
        channels_newest = channels_all.sorted('create_date', reverse=True)[:3]
    ...
    def check_dieu_kien_hoc(self, slide):

        if not slide.channel_id.can_upload \
                and not request.env.user.has_group('website_slides.group_website_quan_tri_vien_tu_xa') \
                and not request.env.user.has_group('website_slides.group_website_quan_tri_vien_chinh_quy') \
                and not request.env.user.has_group('website_slides.group_website_quan_tri_vien_vlvh') \
                and not request.env.user.has_group('website_slides.group_website_quan_tri_vien') \
                and not request.env.user.has_group('website_slides.group_website_slides_officer'):
            slide_partners = http.request.env['slide.slide.partner'].sudo().search([
                ('channel_id.id', '=', slide.channel_id.id),
                ('partner_id.id', '=', request.env.user.partner_id.id),
                ('slide_id.id', '=', slide.ke_tiep_cua.id)])
            print(slide_partners.completed)
            if slide_partners.slide_id.is_published and not slide_partners.completed:
                print("Bạn không đủ điều kiện học slide này")
                return False
            return True
        return True
    ...
    def slide_view(self, slide, **kwargs):
        ...
        # _logger.info(request.render("website_slides.slide_main", values))
    ...
    def slide_get_pdf_content(self, slide):
        # self.check_dieu_kien_hoc(slide)
        print("đang lấy PDF")
    ...
    def slide_get_image(self, slide_id, field='image_128', width=0, height=0, crop=False):
        ...
        # self.check_dieu_kien_hoc(slide_id)
        print("load ảnh")
    ...
    # @http.route('/website_scorm_elearning',type='http', auth='user')
    # def block_access(self):
    #     _logger.info('hello from the scorm controller\n\n')
    #     return None
    # --------------------------------------------------
    # TAGS SECTION
    # --------------------------------------------------
    ...
    def slide_quiz_submit(self, slide_id, answer_ids):
        ...
        slide_slide_partner_id = request.env['slide.slide.partner'].sudo().search([('slide_id', '=', slide.id),('partner_id', '=', request.env.user.partner_id.id)])
        for user_answ in user_answers:
            request.env['slide.partner.qa'].create({
                'slide_id': slide.id,
                'channel_id': slide.channel_id.id,
                'partner_id': request.env.user.partner_id.id,
                'quiz_attempts_count': quiz_info['quiz_attempts_count'],
                'answer_ids': user_answ.id,
                'question_id': user_answ.question_id.id,
                'slide_slide_partner_id': slide_slide_partner_id.id,
                'is_correct': user_answ.is_correct,
            })
        ...
        if user_bad_answers and slide.channel_id.kieu_tinh_diem_quiz == '2':
            slide._action_set_quiz_done()
            slide.action_set_completed()
        self._channel_remove_session_answers(slide.channel_id, slide)
        return {
    ...
    def prepare_preview(self, **data):
        ...
        if not document_id:
            ...
            return preview
        existing_slide = Slide.search([('channel_id', '=', int(data['channel_id'])), ('document_id', '=', document_id)],
                                      limit=1)
        if existing_slide:
            preview['error'] = _('This video already exists in this channel on the following slide: %s',
                                 existing_slide.name)
            return preview
    ...
    def _prepare_user_slides_profile(self, user):
        ...
        courses_dang_hoc = courses.filtered(lambda c: c.completion >= 0) - courses_completed
        ...
        values = {
            ...
            'courses_dang_hoc': courses_dang_hoc,
